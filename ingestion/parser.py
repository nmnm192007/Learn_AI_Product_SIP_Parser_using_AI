...
def parse_log_segment(log_generator: Generator[str, None, None]) -> Dict[str, Any]:
    ...
    for msg in group_messages(log_generator):
        ...
        for line in lines:
            ...
            elif line.startswith("SIP/2.0") and (
                " ".join(line.split()[1:3]) in SIP_RESPONSES
            ):
                ret_dict["sip_msg"] = " ".join(line.split()[1:3]).upper()
            elif (
                line.startswith("SIP/2.0")
                and " ".join(line.split()[1:4]) not in SIP_RESPONSES
            ):
                len_line = len(line.split()) if (len(line.split()) > 1) else 0
                ret_dict["sip_msg"] = (
                    " ".join(line.split()[1:len_line]).upper() if (len_line > 0) else ""
                )
